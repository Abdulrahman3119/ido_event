...
// Migrated from Desk Client Script(s) — IDO Attendance (Form)

// --- IDO Attendance — IDO Form ---
...
frappe.ui.form.on("IDO Attendance", {
	async validate(frm){ if(await frappe.db.exists("IDO Attendance",{guest:frm.doc.guest,session:frm.doc.session,name:["!=",frm.doc.name]})) frappe.throw("الضيف مسجّل حضوره في هذه الجلسة مسبقًا."); if(!frm.doc.checked_in_at) frm.set_value("checked_in_at",IDO.now()); if(!frm.doc.checked_in_by) frm.set_value("checked_in_by",frappe.session.user); },
	async after_save(frm){ const n=await frappe.db.count("IDO Attendance",{filters:{session:frm.doc.session}}); await frappe.db.set_value("IDO Session",frm.doc.session,"attendance_count",n); const d=(await frappe.db.get_value("IDO Session",frm.doc.session,"day")).message.day; if([1,2,3].includes(d)) await IDO.auto(frm.doc.guest,"day"+d); }
});

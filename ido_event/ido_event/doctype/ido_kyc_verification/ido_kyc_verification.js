...
// Migrated from Desk Client Script(s) — IDO KYC Verification (Form)

// --- IDO KYC Verification — IDO Form ---
...
frappe.ui.form.on("IDO KYC Verification", {
	async validate(frm) {
		if (["approved", "rejected"].includes(frm.doc.status) && !frm.doc.reviewed_at) {
			frm.set_value("reviewed_by", frappe.session.user);
			frm.set_value("reviewed_at", IDO.now());
		}
	},
	async after_save(frm) {
		// Server doc_event already mirrors + auto-advances when approved.
		frappe.show_alert({
			message:
				frm.doc.status === "approved"
					? __("تم اعتماد التحقق وتحديث رحلة الضيف")
					: __("تم حفظ التحقق من الهوية"),
			indicator: "green",
		});
	},
});

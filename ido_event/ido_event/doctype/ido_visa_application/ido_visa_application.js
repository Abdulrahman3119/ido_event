...
// Migrated from Desk Client Script(s) — IDO Visa Application (Form)

// --- IDO Visa Application — IDO Form ---
...
frappe.ui.form.on("IDO Visa Application", {
	async validate(frm) { if(!["not_started","not_required"].includes(frm.doc.status)&&(await IDO.latest("IDO KYC Verification",frm.doc.guest))!=="approved") frappe.throw("قاعدة العمل 002: لا يمكن بدء التأشيرة قبل اعتماد التحقق من الهوية."); if(frm.doc.status==="processing"&&!frm.doc.submitted_at) frm.set_value("submitted_at",IDO.now()); if(frm.doc.status==="issued"&&!frm.doc.issued_at) frm.set_value("issued_at",IDO.now()); },
	async after_save(frm) { await IDO.mirror(frm.doc.guest); if(["processing","docs_required"].includes(frm.doc.status)) await IDO.auto(frm.doc.guest,"visa_processing"); else if(["approved","issued","not_required"].includes(frm.doc.status)) await IDO.auto(frm.doc.guest,"visa_approved"); frappe.show_alert({message:"Journey synced",indicator:"green"}); }
});
